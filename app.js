...
// "let" is used (not "const") because we'll reassign this variable later.
...
function render() {
  ...
  // Loop through every todo and create HTML elements for it.
  todos.forEach(function(todo) {

    // createElement creates a new HTML element — here a <li> (list item).
    const li = document.createElement("li");
    li.className = "item";
    // dataset lets us attach custom data to an element. We store the todo's id
    // so we can find the right todo later when the user clicks delete/checkbox.
    ...
    // addEventListener listens for an event ("change") and runs a function when it fires.
    ...
    // Add the "completed" CSS class to draw a strikethrough when done.
    if (todo.completed) {
  ...
  // Show "1 item left" or "2 items left" — the ternary operator picks the word.
...
function addTodo(title) {
  ...
  // unshift() adds an item to the BEGINNING of an array (opposite of push).
  todos.unshift(newTodo);

  writeToStorage(); // Persist the change
...
function deleteTodo(id) {
  ...
  // !== means "not equal to", so we keep everything that doesn't match.
...
$form.addEventListener("submit", function(e) {
  // By default a form submission reloads the page — preventDefault stops that.
  e.preventDefault();
  ...
  addTodo(title);   // Add the new todo
